...
const Mutations = {
  ...
  async deleteLocation(parent, args, ctx, info) {
    ...
    const location = await ctx.db.query.location({ where }, `{ id, user, geoLocation {id}, markerType {id}}`);
    ...
    await ctx.db.mutation.deleteMarker({ where: { id: location.markerType.id } }, info);

  },
  async deleteShape(parent, args, ctx, info) {
    const where = { id: args.id }
    //   const location = await ctx.db.query.location({where}, `id title`);
    const location = await ctx.db.query.shape({ where }, `{ id, user }`);
    const ownLocation = location.user == ctx.request.userId;
    ...
    return ctx.db.mutation.deleteShape({ where }, info);
